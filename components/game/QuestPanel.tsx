...
export default function QuestPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Sub-tabs */}
      <div className="flex" style={{ borderBottom: "2px solid var(--border-default)" }}>
        {(["quests", "achievements"] as SubTab[]).map((t) => (
          <button
            key={t}
            onClick={() => setSubTab(t)}
            className="font-cinzel px-4 py-2"
            style={{
              fontSize: "0.48rem",
              letterSpacing: "0.12em",
              color: subTab === t ? "var(--gold-light)" : "var(--text-secondary)",
              borderBottom: subTab === t ? "2px solid var(--gold)" : "2px solid transparent",
              marginBottom: -2,
              background: "transparent",
            }}
          >
            {t === "quests" ? "📜 QUÊTES" : "🏆 SUCCÈS"}
          </button>
        ))}
      </div>
                    ...
                    {quest.objectives.map((obj, i) => {
                      ...
                      return (
                        <div
                          ...
                          className="p-2.5"
                          style={{
                            background: done ? "rgba(114,184,96,0.07)" : "var(--surface)",
                            border: `1px solid ${done ? "rgba(114,184,96,0.3)" : "var(--border-subtle)"}`,
                          }}
                        >
                          <div className="flex justify-between items-center mb-1.5">
                            <span className="font-crimson text-sm" style={{ color: done ? "var(--color-heal)" : "var(--text-primary)" }}>
                              {done ? "✓ " : ""}{label} {obj.target}
                            </span>
                            <span className="font-cinzel" style={{ fontSize: "0.48rem", color: done ? "var(--color-heal)" : "var(--text-secondary)" }}>
                          ...
                          <ProgressBar
                            value={pct}
                            height="h-1"
                            color={done ? "bg-green-500" : "bg-amber-600"}
                          />
                        </div>
                      );
                    })}
        ...
        <div className="space-y-2">
          {completedQuests.length === 0 ? (
            <div className="game-card text-center py-10">
              ...
              <p className="font-crimson mt-2" style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Accomplissez des quêtes pour les voir apparaître ici.
              </p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-cinzel" style={{ fontSize: "0.45rem", color: type.color, letterSpacing: "0.1em" }}>
                      {type.icon} {type.label.toUpperCase()}
                    </div>
                    <div className="font-crimson text-sm mt-0.5" style={{ color: "var(--text-primary)" }}>
                      {quest.objectives[0]
                        ? `${OBJECTIVE_LABELS[quest.objectives[0].type] ?? quest.objectives[0].type} ${quest.objectives[0].target}`
                        : quest.questId}
                    </div>
                  </div>
                  <span className="font-cinzel flex-shrink-0" style={{ fontSize: "0.6rem", color: "var(--gold-light)" }}>
                    ✓
                  </span>
                </div>
              );
